...
class Oyun(object):
    ...
    def nesneOlustur(self):
        ...
        self.digerYilan = DigerYilan(self.saha, "#FFFF00", self)
        
        # ANA YILANI OLUŞTUR
        self.anaYilan = AnaYilan(self.digerYilan, self.saha, self.yem, "#00FFFF", 0.1)
...
class AnaYilan(Thread):
    def __init__(self, digerYilan, saha, yem, renk, sureBekleme):
        Thread.__init__(self)
        self.nesneOlustur(digerYilan, saha, yem, renk, sureBekleme)
        self.goster()
    
    
    
    # sınıfın nesnelerini oluştur
    def nesneOlustur(self, digerYilan, saha, yem, renk, sureBekleme):
    ...
    # çarpışma varsa "carp", yem yenmişse "ye",
    # diğer yılan tarafından yem yenmişse "yeDiger", diğer durumlarda "hic" döndürür
    def durumaBak(self):
    ...
    # yemin yerini değiştir
    def yemDegistir(self):
        # yeni yem koy
        self.yem.yerAyarla(self.saha.gen, self.saha.yuk, self.koorlar,
                           self.digerYilan.koorlar)
        # yemi göster
        self.yem.goster()
...
class DigerYilan(object):
    def __init__(self, saha, renk, oyun):
        self.nesneOlustur(saha, renk, oyun)
        self.goster()
    
    
    
    # sınıfın nesnelerini oluştur
    def nesneOlustur(self, saha, renk, oyun):
        self.saha = saha
        self.renk = renk
        self.oyun = oyun
        self.canliMi = True
    ...
    def goster(self):
        ...
        if len(self.koorlar) > len(self.koorlarOnceki):
            ...
            # ve ayrıca yem yendiği için yemin yerini değiştir
            self.oyun.yem.yerAyarla(self.oyun.saha.gen, self.oyun.saha.yuk,
                                    self.oyun.anaYilan.koorlar, self.koorlar)
            self.oyun.yem.goster()
...
class Yem(object):
    ...
    # yemin yerini rasgele olarak ayarla
    def yerAyarla(self, genSaha, yukSaha, koorlarYilan, koorlarDigerYilan):
        self.x = randrange(genSaha)
        self.y = randrange(yukSaha)
        while [self.x, self.y] in koorlarYilan or [self.x, self.y] in koorlarDigerYilan:
            self.x = randrange(genSaha)
            self.y = randrange(yukSaha)
...
class PencereNet(Toplevel):
    ...
    def aracYerlestir(self):
        ...
        Label(cerceve1, text = "Sunucu IP: ").pack(side = LEFT)
        Label(cerceve2, text = "Sunucu port: ").pack(side = LEFT)
        Label(cerceve3, text = "Istemci IP: ").pack(side = LEFT)
        Label(cerceve4, text = "Istemci port: ").pack(side = LEFT)
        # girişleri yerleştir
        self.girisIPSunucu = Entry(cerceve1)
        self.girisIPSunucu.insert(END, "127.0.0.1")
        self.girisIPSunucu.pack(side = LEFT)
        self.girisPortSunucu = Entry(cerceve2)
        self.girisPortSunucu.pack(side = LEFT)
        self.girisIPIstemci = Entry(cerceve3)
        self.girisIPIstemci.insert(END, "127.0.0.1")
        self.girisIPIstemci.pack(side = LEFT)
        self.girisPortIstemci = Entry(cerceve4)
    ...
    def bilgiAlIsle(self):
        ...
        if self.soket < 0:
            ...
            exit()
        # bu soketi sunucunun IP ve port una bağla
        self.soket.bind((self.IPSunucu, self.portSunucu))
        
        # gönderici ve alıcıyı oluştur
        self.gonderici = Gonderici(self.soket, self.IPIstemci, self.portIstemci,
                                   self.anaYilan, self.yem)
        self.alici = Alici(self.soket, self.digerYilan)
...
class Gonderici(Thread):
    def __init__(self, soket, IP, port, anaYilan, yem):
        Thread.__init__(self)
        self.nesneOlustur(soket, IP, port, anaYilan, yem)
    
    
    
    # sınıfın nesnelerini oluştur
    def nesneOlustur(self, soket, IP, port, anaYilan, yem):
        self.soket = soket
        self.IP = IP
        self.port = port
        self.anaYilan = anaYilan
    ...
    def run(self):
        while True:
            ...
            # ana yılanın ve yemin koordinatlarını içerir
            strGonder = "yilan:" + str(self.anaYilan.koorlar) + "yem:" +\
                        "[" + str(self.yem.x) + ", " + str(self.yem.y) + "]"
            # ana yılanın ve yemin koordinatlarını istemciye gönder
...
# alıcının sınıfı
class Alici(Thread):
    def __init__(self, soket, digerYilan):
        Thread.__init__(self)
        self.nesneOlustur(soket, digerYilan)
    
    
    
    # sınıfın nesnelerini oluştur
    def nesneOlustur(self, soket, digerYilan):
        self.soket = soket
        self.digerYilan = digerYilan
    ...
    def run(self):
        while True:
            ...
            kacByte = 40000
            # veriyi al
            # bu veri istemciden geldiğine göre sadece oradaki ana yılanın
            ...
            # VERİYİ İŞLE
            # veriden "yilan:" ön ekini çıkar
            veri = veri[6:]
            # yeni koordinatları liste haline getir
            koorlarYeni = eval(veri)
            # buradaki diğer yılanın koordinatlarını yeni koordinatlar haline getir
            self.digerYilan.koorlar = koorlarYeni
            # diğer yılanı göster
